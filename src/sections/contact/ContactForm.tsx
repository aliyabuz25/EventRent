...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.phone || !formData.email || !formData.message) {
      setError(t(locale, s.errorRequired));
      return;
    }
    setLoading(true);
    setError(null);
    try {
      ...
      setError(`${t(locale, s.errorRequired)}: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const infoCards = [
    { icon: Phone, title: t(locale, s.infoPhone), value: t(locale, s.infoPhoneValue), sub: t(locale, s.infoPhoneSub) },
    { icon: Mail, title: t(locale, s.infoEmail), value: t(locale, s.infoEmailValue), sub: t(locale, s.infoEmailSub) },
    { icon: MapPin, title: t(locale, s.infoAddress), value: t(locale, s.infoAddressValue), sub: t(locale, s.infoAddressSub) },
  ];
      ...
      <div className="lg:col-span-2 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[40px] p-8 md:p-16 shadow-2xl shadow-black relative overflow-hidden">
        ...
        {success ? (
          <div className="flex flex-col items-center justify-center py-12 text-center space-y-6">
            ...
            <div>
              <h3 className="text-2xl font-black tracking-tight text-white">{t(locale, s.successTitle)}</h3>
              <p className="text-white/50 mt-2">{t(locale, s.successBody)}</p>
            </div>
            <button
            ...
            >
              {t(locale, s.resetButton)}
            </button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-12" aria-label="contact-form">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-12">
              <div className="space-y-4">
                <label htmlFor="contact-name" className="text-[10px] font-bold text-white/50 uppercase tracking-[0.3em] ml-1">{t(locale, s.labelName)}</label>
                <input
                  id="contact-name" name="name" type="text" autoComplete="name" required
                  value={formData.name}
                  onChange={e => setFormData({ ...formData, name: e.target.value })}
                  placeholder={t(locale, s.placeholderName)}
                  className={inputClassName}
                />
              </div>
              <div className="space-y-4">
                <label htmlFor="contact-phone" className="text-[10px] font-bold text-white/50 uppercase tracking-[0.3em] ml-1">{t(locale, s.labelPhone)}</label>
                <input
                  id="contact-phone" name="phone" type="tel" autoComplete="tel" inputMode="tel" required
                  value={formData.phone}
                  onChange={e => setFormData({ ...formData, phone: e.target.value })}
                  placeholder={t(locale, s.placeholderPhone)}
                  className={inputClassName}
                />
              </div>
            </div>
            <div className="space-y-4">
              <label htmlFor="contact-email" className="text-[10px] font-bold text-white/50 uppercase tracking-[0.3em] ml-1">{t(locale, s.labelEmail)}</label>
              <input
                id="contact-email" name="email" type="email" autoComplete="email" required
                value={formData.email}
                onChange={e => setFormData({ ...formData, email: e.target.value })}
                placeholder={t(locale, s.placeholderEmail)}
                className={inputClassName}
              />
            </div>
            <div className="space-y-4">
              <label htmlFor="contact-message" className="text-[10px] font-bold text-white/50 uppercase tracking-[0.3em] ml-1">{t(locale, s.labelMessage)}</label>
              <textarea
                id="contact-message" name="message" rows={4} required
                value={formData.message}
                onChange={e => setFormData({ ...formData, message: e.target.value })}
                placeholder={t(locale, s.placeholderMessage)}
            ...
            >
              ...
              <span className="relative z-10 flex items-center gap-4">
                {loading ? (
                  ...
                  <>{t(locale, s.submitButton)} <ArrowUpRight aria-hidden="true" className="w-6 h-6 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" /></>
                )}
              </span>
            </button>
          </form>
        ...
        {infoCards.map((info, i) => (
          <div key={i} className="relative p-10 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[40px] space-y-6 hover:bg-white/10 hover:border-white/20 transition-all duration-700 group overflow-hidden">
            ...
            <div className="relative z-10 space-y-2">
              <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/60">{info.title}</h4>
              <p className="text-xl md:text-2xl font-bold tracking-tight text-white">{info.value}</p>
              <p className="text-sm text-white/70 font-light">{info.sub}</p>
            </div>
          </div>
        ))}
      </div>
